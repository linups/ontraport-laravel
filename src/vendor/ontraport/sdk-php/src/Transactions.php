...
class Transactions extends BaseApi
{
     ...
     * @return string JSON formatted HTTP response
     */
    public function processManual($requestParams, $raw = false)
    {
        $options["headers"] = self::retrieveContentTypeHeader(self::CONTENT_TYPE_JSON);
        ...
        if (isset($raw) && $raw != '') {
            return $this->client->request($requestParams, $this->_mainTransactionEndpoint . "/" . self::PROCESS_MANUAL, "post", $requiredParams, $options, $raw);
        } else {

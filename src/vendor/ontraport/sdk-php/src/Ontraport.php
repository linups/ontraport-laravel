...
class Ontraport
{
     ...
     * @return mixed
     */
    public function request($requestParams, $url, $method, $requiredParams, $options, $raw = false)
    {
        $client = $this->getHttpClient();
        $url = $this->buildEndpoint($url);

        if(isset($raw) && $raw != '') {
            return $client->httpRequest($requestParams, $url, $method, $requiredParams, $options, $raw);
        } else {
